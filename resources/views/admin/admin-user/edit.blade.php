                ...
                <div class="col">
                    <div class="card dashboard-custom-card">
                        <div class="card-body">
                            ...
                            <form id="adminUserForm" action="{{ route('admin.user.update', $data->id) }}" method="post"
                                            ...
                                            <select class="form-control multiselect select2" id="role_id" name="role_id[]" multiple required>
                                                @foreach($roles as $role)
                                                    <option value="{{ $role->id }}"
                                                            {{ in_array($role->id, old('role_id', $role_ids)) ? 'selected' : '' }}>
                                                        {{ $role->display_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <div class="mt-2">
                                                @php
                                                    $imageUrl = $data->profile_pic
                                                        ? Storage::url($data->profile_pic)
                                                        : asset('defaults/noimage/no_img.jpg');
                                                @endphp
                                                <img id="profile_preview" src="{{ $imageUrl }}" alt="Profile Preview"
